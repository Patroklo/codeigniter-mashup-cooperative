...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
            ...
            $user = $this->correcaminos->beep($this->ion_auth_model->user_object)->where($this->config->item('identity', 'ion_auth'), $identity)->where('active', 1)->row();  //changed to get_user_by_identity from email
	...
	public function forgotten_password_complete($code)
	{
		...
		$profile  = $this->correcaminos->beep($this->ion_auth_model->user_object)->where('forgotten_password_code', $code)->row(); //pass the code to profile
	...
	public function in_group($check_group, $id=false, $check_all = false)
	{
		...
		if($id == FALSE)
		{
			return $this->auth->in_group($check_group, $check_all);
		}
